...
(!function(global,factory){
	...
	let ty={a:function(v){return Array.isArray(v);},n:function(v){return typeof v==='number';},s:function(v){return typeof v==='string';},o:function(v){return typeof v==='object';},b:function(v){return typeof v==='boolean';},f:function(v){return typeof v==='function';},u:function(v){return typeof v==='undefined';}},
		ga=function tg(a,b,c,d){if(ty.u(gc.change))gc.change={};if(ty.u(gc.change[a]))gc.change[a]={};if(ty.u(gc.change[a][b]))gc.change[a][b]={};gc.change[a][b][c]=d;},
		gb=function tg(a,b,c,d){Object.defineProperty(a,b,{value:c,writable:false,enumerable:false,configurable:d||false});},
		gc=function tg(a,b){return fn.re.dom(a,b);},
		gd={},ge={},gf={},gg={},gh={},gi=[],gj=[];
			...
			if(ty.o(b)){
				if(ty.n(b.length)){
					for(let i=0;i<b.length;i++){
						if(gi.indexOf(ww)===-1)
							gi.push(ww);
						let va=/\[object [a-zA-Z]+\]/.test(String(ww))?ww+gi.indexOf(ww):ww,
						fa=function(ev){
							for(let j=0;j<gh[va][c].length;j++){
						...
						if(ty.u(gh[va][c]))
							gh[va][c]=[];
						gh[va][c].push(function(ev){a(ev);});
						ww.addEventListener(c,fa,b||false);
					}
				}else 
					a=function(f,c){return function tg(){return f.apply(c,arguments);};}(a,b);
			}else a();
			return a;
		}
		if(ty.o(a)){
			let va=String(a).match(/\[object HTML([a-zA-Z]+).*/),
				vb=String(a).match(/\[object ([A-Z][a-z]+).*/);
			this.a=a;
			if(ty.o(a.a)||(va===null&&vb===null))return a;
			if(a===ww){this.selector='window';this.length=1;this.a.length=1;return;}
			if(a===ww.document){this.selector='document';this.length=1;this.a.length=1;return;}
			if(ty.s(a.href)){this.selector='a';this.length=1;return;}
			if(ty.a(va)){this.selector=va[1].toLowerCase();this.length=a.length;return;}
			if(ty.a(vb)){if(vb[1]==='Object')return a;this.selector=vb[1].toLowerCase();this.length=a.length;return;}
		}
		if(ty.s(a)){
			let le=function(st){
				let se=function(sr){
					let dp=function(av){
						let rg=[/^(.+)\[(\d+)-(\d+)\]$/im,/^(.+)\((\d+),(\d+)\)$/im,/^(.+)\[(\d+)-\]$/im,/^(.+)\[(\d+)\]$/im,/^(.+)!(\d+)$/im,/^(.+):(\d+)$/im];
						for(let i=0;i<rg.length;i++){
							let mc=av.match(rg[i]);
							if(ty.a(mc)){
								mc=mc.map(function(aa){let va=+aa;return isNaN(va)?aa:va;});
								return[[[mc[2],mc[3]],0,sr=mc[1]],[[mc[2],mc[3]],1,sr=mc[1]],[[mc[2],mc[3]],2,sr=mc[1]],[mc[2],4,sr=mc[1]],[mc[2],3,sr=mc[1]],[mc[2],4,sr=mc[1]]][i];
							}
						}
						return false;
					}(sr),rs=ww.document.querySelectorAll(sr);
					if(ty.a(dp))
						switch(dp[1]){
							case 0:{let ar=[];if(dp[0][0]>=0&&dp[0][1]>=0&&dp[0][0]<=dp[0][1])for(let i=dp[0][0];i<=dp[0][1];i++)if(ty.o(rs[i]))ar.push(rs[i]);rs=ar;};break;
							case 1:{let ar=[];if(dp[0][0]>=0&&dp[0][1]>=0)for(let i=dp[0][0],j=0;j<dp[0][1];i++,j++)if(ty.o(rs[i]))ar.push(rs[i]);rs=ar;};break;
							case 2:{let ar=[];if(dp[0][0]>=0&&!dp[0][1])for(let i=dp[0][0];i<=rs.length;i++)if(ty.o(rs[i]))ar.push(rs[i]);rs=ar;};break;
							case 3:{let ar=[];for(let i=0;i<rs.length;i++)if(ty.o(rs[i]))ar.push(rs[i]);ar.splice(dp[0],1);rs=ar;};break;
							case 4:rs=rs[dp[0]];break;
						}
					return rs;
				},lb=st.split(/(?!\([^)]*),(?![^(]*\))/);
				if(lb.length>1){
			...
			tg===null||tg.length===0?gc.fn.DOMCL===true?(this.a=null,this.length=0):(ww.document.addEventListener('DOMContentLoaded',this.a=function tg(){
				let rs=le(a).length===0?null:le(a);
				for(let i in gg){
					for(let j in gg[i]){
						if(ty.s(gg[i][j]))
							gc(i)[j](gg[i][j]);
			...
			},b&&true),this.length=undefined):(this.length=ty.u(tg.length)?1:tg.length||0,this.length===1&&ty.o(tg[0])?this.a=tg[0]:ty.o(tg)?this.a=tg:(this.a=null,this.length=0));this.selector=a;
		}
	});
	gb(fn.re,'dom',function(a,b){
		let ba={
			addClass:function(c){
				if(ty.f(this.a))bc.addClass(a,c,'addClass');
				...
				if(this.a.className==''||ty.u(this.a.className))
					this.a.className=c;
				else 
				...
				if(ty.o(c)){
					let z={
						colorRotate:function(f){
							let vb=(va.a.style.backgroundColor||'rgb(255,255,255)').match(/[rgb|rgba]\((\d+),[ ]?(\d+),[ ]?(\d+)*/),
						...
						opacity:function(f){
							let vb=va.a.opacity=va.a.style.opacity=new fn.re(a,b).css('opacity')||1,
							...
							let vc=vb>f?(d/((vb-f)/e)):(d/((f-vb)/e));
							if(vb!=f)fa();
						},
						scrollTop:function(f){
							let vb=va.animate('scrollTop'),
					...
					for(let i in c)if(ty.f(z[i]))z[i](c[i]);
				}
				if(ty.s(c)){
					if(c==='scrollTop'&&ty.u(d)&&ty.u(e))return ww.pageYOffset?ww.pageYOffset:ww.document.documentElement.scrollTop?ww.document.documentElement.scrollTop:ww.document.body.scrollTop;
					if(c==='scrollLeft'&&ty.u(d)&&ty.u(e))return ww.document.documentElement.scrollLeft?ww.document.documentElement.scrollLeft:ww.document.body.scrollLeft;
			...
			append:function(c,d){
				...
				if(ty.s(c)||ty.o(c)){
					let fa=function(aa){
						let va;
						if(ty.s(c)){
							va=ty.u(d)?ww.document.createElement('DIV'):ww.document.createElement(d);
							va.innerHTML=c;
						...
						if(ty.o(c)){
							if(c.length>0){
								for(let i=0;i<c.length;i++){
									if(ty.s(c[i])){
										ww.document.createElement('DIV').innerHTML=c[i];
										ga(aa,'append','appendChild',va);
										aa.appendChild(va);
									}
									if(ty.o(c[i])){
			...
			attr:function(c,d){
				if(ty.f(this.a))bc.attr(a,c,d,'attr');
				if(this.a==null)return this;
				let fa=function(c,d,e){
					if(ty.s(d)){
			...
			createElement:function(c){
				let va=ww.document.createElement(a);
				if(ty.o(c))
					for(let i in c)
						if(ty.s(c[i]))
			...
			css:function(c,d){
				if(ty.f(this.a))bc.css(a,c,d,'css');
				if(this.a==null)return this;
				if((ty.s(c)||ty.o(c))&&(ty.s(d)||ty.u(d))){
					if(ty.o(this.a)&&this.a!=null){
						let fa=function(aa){
							if(ty.s(c)&&ty.s(d)){
								aa.style[c]=d;
								ga(a,'css',c,d);
					...
					if(ty.s(c)&&ty.u(d)){
						let fa=function(aa){
							if(ty.o(aa.style))
								if(!ty.u(aa.style[c])&&aa.style[c]!=''&&aa.style[c]!='auto')
									return aa.style[c];
							if(!ty.u(aa[c])&&aa[c]!=''&&aa[c]!='auto')
								return aa[c];
							return false;
			...
			drgdrp:function(c){
				...
				this.a.onmousedown=function(ev){
					let va=ww.document.elementFromPoint(ev.clientX,ev.clientY),
					...
					ww.document.onmouseup=function(ev){ww.document.onmousemove=null;ww.document.body.onmousedown=null;};
					ww.document.onselectstart=function(ev){ev.preventDefault();};
					ww.document.ondragstart=function(ev){ev.preventDefault();};
					ww.document.onmousemove=function(ev){
			...
			getCoordinates:function(){
				if(ty.f(this.a)||this.a==null)return this;
				return function(aa){
					let va=aa.getBoundingClientRect(),
						vb=ww.document.body,
						vc=ww.document.documentElement,
			...
			getFocused:function(c){
				if(this.a!=ww)return;
				let fa=function(){
					if((ww.document.visibilityState||ww.document.webkitVisibilityState)==='hidden')return false;
					if((ww.document.visibilityState||ww.document.webkitVisibilityState)==='visible')return true;
				};
				if(ty.f(c)){
					if(!fa())ww.document.addEventListener('focus',function(){c();},false);
					if(fa())c();
				}
			...
			getIndex:function(){
				if(ty.f(this.a)||this.a==null)return this;
				for(let i=0;i<new fn.re(this.a.tagName,b).a.length;i++)
					if(new fn.re(this.a.tagName,b).a[i]===this.a)
						return i;
				return this;
			},
			html:function(c){
				if(ty.f(this.a))bc.html(a,c,'html');
				if(this.a==null)return this;
				if(ty.o(c)||ty.s(c)){
					let fa=function(aa){
						if(ty.o(c)){
				...
				if((ty.s(c)&&ty.f(d)&&ty.u(e))||(ty.s(c)&&ty.s(d)&&ty.f(e))){
					...
					if(/\[object [a-zA-Z]+\]/.test(String(a))){
						...
						if(/(.*)\[[a-zA-Z]+ ([a-zA-Z]+)\](.*)/.exec(vc)!=null){
							let vd=/(.*)\[[a-zA-Z]+ ([a-zA-Z]+)\](.*)/.exec(vc);
							vc=vd[1]+vd[2]+vd[3];
						}
						if(gi.indexOf(a)!=-1){
							for(let i=0;i<gh[vc][c].length;i++){
						...
						if(ty.o(gh[vb])){
							if(ty.o(gh[vb][c])){
								for(let i=0;i<=gh[vb][c].length;i++){
			...
			on:function(c,d,e){
				...
				if(this.a==null)return this;
				let va=/\[object [a-zA-Z]+\]/.test(String(a)),
					ve=[
				...
				if(ve[0]||ve[1]||ve[2]||ve[3]){
					...
					if(ty.u(gh[vb][c]))
						gh[vb][c]=[];
					gh[vb][c]['a']=this.a;
					if(ty.n(this.a.length)&&this.a.length>1)
						for(let i=0;i<this.a.length;i++)
							this.a[i]['on'+c]=fa;
					else 
						this.a['on'+c]=fa;
			...
			prop:function(c,d){
				if(ty.f(this.a))bc.prop(a,c,d,'prop');
				if(this.a==null)return this;
				if(ty.s(c)&&!ty.u(d)){
					ga(a,'prop',c,d);
					this.a[c]=d;
			...
			remove:function(c){
				...
				if(ty.s(c)||ty.o(c)){
					let fa=function(aa){
						if(ty.s(c)){
							ww.document.createElement('DIV').innerHTML=c;
			...
			removeClass:function(c){
				if(ty.f(this.a))bc.removeClass(a,c,'removeClass');
				if(this.a==null)return this;
				if(ty.s(c)&&this.a.className!=''){
					let va=this.a.className.split(' ');
			...
			submit:function(c){
				if(ty.f(this.a))bc.submit(a,c,'submit');
				if(this.a==null)return this;
				...
				if(ty.s(c)){
					let fa=function(aa){
						for(let i=0;i<aa.length;i++){
							let va=aa[i],
								fb=function(r,l){
									if(va.value===''||va.value.length<l||!r.test(String(va.value)))
										return false;
								},
								fc=function(){
									va.focus();
									gc(va).css('background','rgb(255,225,225)').animate({colorRotate:[255,255,255]},20);
								};
							switch(va.getAttribute('name')){
								case 'text':if(fb(/^[\w.@+ -]{4,50}$/,1)===false){fc();return false;}break;
								case 'login':if(fb(/^[\w.@+ -]{4,50}$/,4)===false){fc();return false;}break;
								case 'password':if(fb(/^[\w ]{4,50}$/,4)===false){fc();return false;}break;
								case 'email':if(fb(/^[\w.@ _-]{4,50}$/,5)===false){fc();return false;}break;
								case 'mobile':if(fb(/^[\w+ -]{4,50}$/,3)===false){fc();return false;}break;
								default:'';
							}
						}
						return true;
					};
					this.a.onsubmit=function(){return false;};
					if(fa(this.a)===true){
		...
		bb=[
			...
			function(c,d,e,f){
				...
				if(ty.o(d)&&ty.u(e))for(let i in d)gg[c][f][i]=d[i];
		...
		bc={
			addClass:function(c,d){bb[3](c,d,'addClass');},
			attr:function(c,d,e){bb[1](c,d,e,'attr');},
			css:function(c,d,e){bb[1](c,d,e,'css');},
			html:function(c,d){bb[0](c,d,'html');},
			on:function(c,d,e,f){bb[2](c,d,e,f,'on');},
			prop:function(c,d){bb[0](c,d,'prop');},
			removeClass:function(c,d){bb[3](c,d,'removeClass');},
			submit:function(c,d){bb[0](c,d,'submit');},
			val:function(c,d){bb[0](c,d,'val');}
		};
		fn.re.prototype=ty.f(Proxy)?new Proxy(ba,{
			get:function(t,k){
				if(a===''||a===null||ty.n(a)||String(a)==='[object Object]')return a;
				if(ty.s(a)||ty.o(a)){
					let va=new fn.re(a,b).a;
					if(ty.u(bc[k])&&ty.f(va))return null;
					if(ty.f(va))return function tg(c,d,e,f){if(ty.f(bc[k]))bc[k](a,c,d,k);return this};
					if(ty.o(va))return t[k];
				}
			}
		}):ba;
		for(let i in ge)if(ty.u(ba[i]))fn.re.prototype[i]=ge[i];return''===a||null===a||ty.n(a)||ty.b(a)||ty.u(a)?a:new fn.re(a,b);
	});
	return(Object.setPrototypeOf||function(a,b){for(let i in b)gb(a,i,b[i]);a.__proto__=b;return a;})(gc,{
		ajax:function(c,d){
			if(ty.o(c)){
				let va=this.getXmlHttp(),
					a_2=ty.f(c[2])?c[2]:undefined,
					a_3=ty.f(c[3])?c[3]:undefined,
					a_4=ty.f(c[4])?c[4]:undefined;
				va.open(c.type,c.url,true);
				if(ty.o(c.headers))
					for(let i in c.headers)
		...
		databaseSelect:function(c,d){
			if(d.split('=')[0]==='id'){
				...
				for(let i=0;i<ww.databasedata[c].length;i++)
		...
		databaseUpdate:function(c,d,e){
			...
			_:for(let i=0;i<ww.databasedata[c].length;i++){
				for(let j in ww.databasedata[c][i]){
					for(let k in ww.databasedata[c][i]){
		...
		hotkey:function(c,d,e){
			...
			new fn.re(ww).on(c,function(ev){
				if(gj.indexOf(ev.keyCode)===-1)
					gj.push(ev.keyCode);
				if(ty.o(gf[gj.join('+')])){
					for(let i=0;i<gf[gj.join('+')].length;i++)
						gf[gj.join('+')][i]();
					gj=[];
					ev.preventDefault();
				}
		...
		notifi:function(c){
			if(!('Notification' in ww))
				...
				if(Notification.permission==='granted'&&!ty.u(c)&&c!='')
					new Notification(c);
				else 
		...
		parserUrl:function(c){
			...
			for(let i=0;i<va.directory.length;i++)
				if(ty.u(va.directory[i])||va.directory[i]==='')
			...
			va.query={};
			va.file=va.file||'';
			va.path=va.parameterst&&va.parameterst!=''?[va.pathname,va.parameterst].join('?'):va.pathname;
			va.urlnodom=va.strdir+va.file+va.parameterst+va.hash;
			if(va.directory[0]===''||ty.u(va.directory[0]))va.directory='';
			if(va.parameter[0]===''||ty.u(va.parameter[0]))va.parameter='';
			if(va.parameterns[0]===''||ty.u(va.parameterns[0])){va.parameterns='';}
			for(let i=0;i<va.parameter.length;i++)
				va.query[va.parameter[i].split('=')[0]]=va.parameter[i].split('=')[1];
			va.mod=(function(){
				for(let i=va.parameter.length;i>0;i--)
		...
		require:function(c,d){
			...
			if(ty.o(c)&&c.length>0){
				for(let i=0;i<c.length;i++)
					for(let j=0;j<c.length;j++)
						if(i!=j&&c[i]===c[j])
							c.splice(j,1);
				_:for(let i=0;i<c.length;i++){
					for(let j=0;j<va[0].length;j++){
						if(ty.a(c[i].match(va[0][j]))){
							for(let k=0;k<va[1][j].a.length;k++)
